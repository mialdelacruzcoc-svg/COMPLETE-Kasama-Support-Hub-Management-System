...
// Build normalized FAQ categories so filter buttons remain consistent.
$category_aliases = [
    'academic' => 'Academic',
...
$normalize_category = function($value) use ($category_aliases) {
    $trimmed = trim((string)$value);
    $lower = strtolower($trimmed);
    return $category_aliases[$lower] ?? ucwords($lower);
};

// Get all categories for filter.
$categories_query = "SELECT DISTINCT category FROM faqs WHERE category IS NOT NULL AND TRIM(category) != '' ORDER BY category";
$categories_result = mysqli_query($conn, $categories_query);

$categories = [];

// Always include known categories so Personal Support is available in filter buttons.
foreach (['Academic', 'Enrollment', 'Financial', 'Personal Support', 'Others'] as $base_category) {
    $categories[$base_category] = $base_category;
}

if ($categories_result) {
    while ($cat = mysqli_fetch_assoc($categories_result)) {
        $normalized = $normalize_category($cat['category'] ?? '');
        if ($normalized !== '') {
...
while ($row = mysqli_fetch_assoc($faqs_result)) {
    $row['normalized_category'] = $normalize_category($row['category'] ?? '');
    $faqs_array[] = $row;
}
?>
<!DOCTYPE html>
...
?>
                    <button class="category-btn" data-category="<?php echo htmlspecialchars(strtolower($cat_label)); ?>">
            ...
            <div class="faq-list" id="faqList">
                <?php if (count($faqs_array) > 0): ?>
                    <?php foreach ($faqs_array as $row): ?>
                        <div class="faq-item" data-id="<?php echo $row['id']; ?>" data-category="<?php echo htmlspecialchars(strtolower($row['normalized_category'])); ?>" data-question="<?php echo htmlspecialchars(strtolower($row['question'])); ?>" data-answer="<?php echo htmlspecialchars(strtolower($row['answer'])); ?>">
                            <button class="faq-question" onclick="toggleFaq(this, <?php echo $row['id']; ?>)">
                                ...
                                <div class="faq-meta">
                                    <span class="faq-category-tag"><?php echo htmlspecialchars($row['normalized_category']); ?></span>
