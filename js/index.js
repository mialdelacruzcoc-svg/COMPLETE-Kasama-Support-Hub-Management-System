...
// --- MATH CAPTCHA LOGIC ---
    ...
    function generateCaptcha() {
        ...
        const answerField = document.getElementById('captchaAnswer');
        answerField.value = '';
        answerField.classList.remove('captcha-correct', 'captcha-incorrect');
    }
    generateCaptcha();
    document.getElementById('captchaRefresh').addEventListener('click', generateCaptcha);

    // --- CAPTCHA REAL-TIME FEEDBACK ---
    document.getElementById('captchaAnswer').addEventListener('input', function() {
        var val = this.value.trim();
        this.classList.remove('captcha-correct', 'captcha-incorrect');
        if (val === '') return;
        var num = parseInt(val, 10);
        if (!isNaN(num) && num === (captchaA + captchaB)) {
            this.classList.add('captcha-correct');
        } else {
            this.classList.add('captcha-incorrect');
        }
    });
